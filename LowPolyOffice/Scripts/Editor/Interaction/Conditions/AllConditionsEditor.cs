...
public class AllConditionsEditor : Editor
{
    ...
    public override void OnInspectorGUI ()
    {
        ...
        if (GUILayout.Button ("add condition"/*, GUILayout.Width (buttonWidth)*/)) {
            AddCondition (newConditionDescription);
            newConditionDescription = "New Condition";
        }
        if (GUILayout.Button("add item condition"/*, GUILayout.Width(buttonWidth * 10f)*/)) {
            AddItemCondition(newConditionDescription, "new item", 1);
            newConditionDescription = "New Condition";
        }
    ...
    private void AddCondition(string description) {
        ...
        AddConditionToList(description, newCondition);
    }

    private void AddItemCondition(string description, string uniqueName, int amount) {
        // If there isn't an AllConditions instance yet, put a message in the console and return.
        if (!AllConditions.instance) {
            Debug.LogError("AllConditions has not been created yet.");
            return;
        ...
        // Create a condition based on the description.
        Condition newCondition = HaveItemConditionEditor.CreateItemCondition(description, uniqueName, amount);
        AddConditionToList(description, newCondition);
    }

    private static void AddConditionToList(string description, Condition newCondition) {
